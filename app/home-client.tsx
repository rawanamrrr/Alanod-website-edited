...
import { openWhatsAppOrder, openWhatsAppWithMessage, getCollectionOrderMessage } from "@/lib/whatsapp"
import { useSiteImages, SITE_IMAGE_KEYS } from "@/lib/site-images"
...
type HomeCollection = { id: string; name: string; imageUrl: string }

export default function HomeClient({
  initialSiteImages,
  initialCollections,
}: {
  initialSiteImages: Record<string, string>
  initialCollections: HomeCollection[]
}) {
  ...
  const [collections, setCollections] = useState<HomeCollection[]>(initialCollections)
  const [loadingCollections, setLoadingCollections] = useState(initialCollections.length === 0)
  ...
  const { getImage } = useSiteImages(initialSiteImages)
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      >
        ...
        >
          <Image
            src={getImage(SITE_IMAGE_KEYS.homeHero)}
      ...
      >
        <div className="container mx-auto px-6">
          ...
          >
            <h2 className="text-2xl md:text-3xl font-light tracking-wider mb-6 font-engravers">{t("collections")}</h2>
